...
if [ "$(osmosisd tx wasm execute $CONTRACT $PAYLOAD --from test_master --chain-id osmosis-dev-1 -y --broadcast-mode block --fees 900stake --output json | jq -r '.code')" = "0" ]; then
    echo "success"
else
    echo "failure"
fi
